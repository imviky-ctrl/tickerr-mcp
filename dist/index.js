...
// ── Tool: list_tools ─────────────────────────────────────────────────────────
server.tool('list_tools', 'List all AI tools monitored by tickerr.ai (42+ tools including ChatGPT, Claude, Gemini, Cursor, GitHub Copilot, etc.)', {}, async () => {
    const data = await fetchJSON('/tools');
    const lines = data.tools.map((t) => `• ${t.name} (slug: ${t.slug}, vendor: ${t.vendor}, category: ${t.category_slug})`);
    return {
        content: [
            {
                type: 'text',
...
server.tool('get_tool_status', 'Get the current operational status, uptime percentage, and response time for an AI tool. Use the tool slug (e.g. "chatgpt", "claude", "cursor", "github-copilot").', { slug: z.string().describe('Tool slug, e.g. "chatgpt", "claude", "cursor"') }, async ({ slug }) => {
    const data = await fetchJSON(`/tools/${slug}/status`);
    const uptimeLine = (label, pct) => pct !== null ? `${label}: ${pct}%` : `${label}: no data`;
    const lines = [
    ...
    if (data.active_incident) {
        ...
        lines.push('', `⚠️ Active incident: ${inc.title}`, `  Severity: ${inc.severity}`, `  Started: ${new Date(inc.started_at).toUTCString()}`, `  Source: ${inc.source}`);
    }
...
server.tool('get_tool_incidents', 'Get recent incidents (outages, degradations) for an AI tool from the last 90 days.', {
    ...
    const data = await fetchJSON(`/tools/${slug}/incidents${qs}`);
    if (data.count === 0) {
        return {
    ...
    for (const inc of data.incidents) {
        ...
        const duration = inc.resolved_at
            ...
            : null;
        lines.push(`### ${inc.title}`, `Severity: ${inc.severity} | ${resolved}${duration ? ` | ${duration}` : ''}`, `Started: ${new Date(inc.started_at).toUTCString()}`);
        if (inc.description)
            lines.push(`Details: ${inc.description}`);
        if (inc.affected_components?.length)
            lines.push(`Affected: ${inc.affected_components.join(', ')}`);
        lines.push('');
    }
    return { content: [{ type: 'text', text: lines.join('\n') }] };
});
// ── Tool: get_pricing ────────────────────────────────────────────────────────
server.tool('get_pricing', 'Get current API pricing for AI models tracked by tickerr.ai. Returns input/output cost per 1M tokens. Optionally filter by tool name or model name.', {
    filter: z
        .string()
        .optional()
    ...
    const rows = models.map((m) => `${m.model_name.padEnd(40)} ${m.tool_name.padEnd(20)} ${fmt(m.input_per_1m).padStart(10)} ${fmt(m.output_per_1m).padStart(10)} ${fmt(m.cached_input_per_1m).padStart(10)}`);
    const text = [
...
// ── Tool: estimate_cost ──────────────────────────────────────────────────────
server.tool('estimate_cost', 'Estimate the API cost for a given number of input and output tokens across all tracked models. Useful for comparing costs before choosing a model.', {
